...
void boottest(char *ifname, uint16 slave, char *filename)
{
	...
	if (ec_init(ifname))
	{
	    ...
	    if ( ec_config_init(FALSE) > 0 )
		{
			...
			/* disable SM1 + SM2 so their offset + length can be changed from
			 * the values set in ec_config_init()
			 */
			uint8_t activate = 0;
			ec_FPWR(ec_slave[slave].configadr, ECT_REG_SM0+6, 1, &activate, EC_TIMEOUTRET);
			ec_FPWR(ec_slave[slave].configadr, ECT_REG_SM1+6, 1, &activate, EC_TIMEOUTRET);
